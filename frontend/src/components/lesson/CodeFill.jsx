...
export default function CodeFill({ lesson }) {
  ...
  const submit = () => {
    ...
    alert(isCorrect ? '🎉 정답입니다!' : '❌ 다시 시도하세요');
  };
      ...
      <div className="codefill-left">
        ...
        <div className="hint-group">
          {lesson.content.hints.map((hint) => {
            const opened = openedHint === hint.level;

            return (
              <div
                key={hint.level}
                className={`hint-dropdown ${opened ? 'open' : ''}`}
              >
                <button
                  className="hint-toggle"
                  onClick={() => setOpenedHint(opened ? null : hint.level)}
                >
                  💡 Hint {hint.level}
                  <span className="hint-arrow">{opened ? '−' : '+'}</span>
                </button>

                {opened && <div className="hint-content">{hint.text}</div>}
              </div>
            );
          })}
        </div>
      ...
      <div className="codefill-right">
        ...
        <div className="code-editor">
          <div className="code-editor-header">
            ...
            <button className="submit-answer-btn" onClick={submit}>
              제출
            </button>
          </div>
