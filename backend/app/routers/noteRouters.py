...
def create_wrong_answer(
    ...
    return create_wrong_answer_service(request, db)
...
@router.get(
    ...
    summary="오답 목록 조회",
    description="오답노트 목록을 조회합니다. track, source_type, is_resolved로 필터링할 수 있습니다.",
)
def get_wrong_answers(
    track: str | None = Query(default=None, description="트랙명: ML, CV, NLP"),
    source_type: str | None = Query(default=None, description="learning 또는 daily"),
...
@router.get(
    ...
    summary="오답 복습 문제 조회",
    description="아직 해결되지 않은 오답을 복습용 문제로 조회합니다.",
)
def get_review_wrong_answers(
    limit: int = Query(default=5, ge=1, le=20, description="조회할 문제 수 - 5문제"),
...
@router.post(
    ...
    summary="오답 복습 제출",
    description="오답 복습 답안을 제출하고 정답 여부를 확인. XP는 부여x",
)
def submit_review_answers(
    request: ReviewSubmitRequest,
    db: Session = Depends(get_db),
):
    return submit_review_answers_service(request, db)
...
def get_wrong_answer_detail(
    ...
    return get_wrong_answer_detail_service(wrong_answer_id, db)
...
def delete_wrong_answer(
    ...
    return delete_wrong_answer_service(wrong_answer_id, db)